...
const default_to_preprocess = true;
...
(function(){
  let todo = (req,res)=>{
    if (req.query.proc==undefined && default_to_preprocess) {
      req.query.proc="preprocess-cl"
    }
    ...
    try {
      ...
      if (req.query.ptb4 && typeof(req.query.proc)=="string" && enable_proc) {
        source=child.execSync("process/".concat(req.query.proc).concat(" -"),{
          //maxBuffer: maxbuf,
          input: source
        }).toString()
      }
